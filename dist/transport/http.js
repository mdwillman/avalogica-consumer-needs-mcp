...
async function handleMcpRequest(req, res, config) {
    ...
    if (req.method === 'POST') {
        await createConsumerNeedsSession(req, res, config);
        return;
    }
    res.statusCode = 400;
 ...
 * @private
 */
async function createConsumerNeedsSession(req, res, config) {
    // ✅ create fresh Avalogica Consumer Needs server instance
    const serverInstance = createStandaloneServer();
    const transport = new StreamableHTTPServerTransport({
        sessionIdGenerator: () => randomUUID(),
        onsessioninitialized: (sessionId) => {
            sessions.set(sessionId, { transport, server: serverInstance });
            // console.error('[Avalogica Consumer Needs MCP] New session created:', sessionId);
        }
    });
    transport.onclose = () => {
        if (transport.sessionId) {
            sessions.delete(transport.sessionId);
            // console.error('[Avalogica Consumer Needs MCP] Session closed:', transport.sessionId);
        }
    };
    try {
        ...
        // console.error('[Avalogica Consumer Needs MCP] Streamable HTTP connection error:', error);
        res.statusCode = 500;
...
function handleHealthCheck(res) {
    ...
    res.end(JSON.stringify({
        status: 'healthy',
        service: 'avalogica-consumer-needs-mcp',
        version: '0.1.0',
...
function logServerStart(config) {
    ...
    // console.error(`[AI Consumer Needs MCP] Server listening on ${displayUrl}`);
