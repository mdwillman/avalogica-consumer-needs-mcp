...
export const searchEdgeCommunitiesTool = {
    definition: {
        ...
        description: "Search early-adopter communities (e.g., Reddit, Hacker News, niche forums, GitHub) for complaints, hacked workflows, and 'somebody please build this' posts. Use this when you want raw, ground-level signals about unmet needs.",
        inputSchema: {
            ...
            properties: {
                query: {
                    type: "string",
                    description: "Description of the need, frustration, or workflow to investigate among early adopters (e.g., 'small business owners frustrated with AI agents for bookkeeping').",
                },
                numResults: {
                    type: "number",
                    minimum: 1,
                    maximum: 10,
                    description: "How many community posts/pages to retrieve (1–10). Defaults to 5 if omitted.",
                },
            },
            required: ["query"],
        },
    ...
    handler: async (args) => {
        ...
        if (args.numResults !== undefined) {
            if (typeof args.numResults !== "number" || Number.isNaN(args.numResults)) {
                throw new McpError(ErrorCode.InvalidParams, "numResults must be a number between 1 and 10 if provided.");
            }
            numResults = Math.min(10, Math.max(1, Math.floor(args.numResults)));
        }
        try {
            const trimmedQuery = args.query.trim();
            console.error(`[AI Consumer Needs MCP] 🔍 search_edge_communities query="${trimmedQuery}" numResults=${numResults}`);
            ...
            console.error("[AI Consumer Needs MCP] ❌ search_edge_communities handler error:", error);
