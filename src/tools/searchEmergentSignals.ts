        ...
        inputSchema: {
            ...
            properties: {
                ...
                resultCategory: {
                    type: "string",
                    description:
                        "Optional Exa result category to filter by (e.g., 'company', 'research_paper', 'news_article', 'github', 'tweet').",
                    enum: [
                        "company",
                        "research_paper",
                        "news_article",
    ...
    handler: async (args: SearchEmergentSignalsArgs): Promise<CallToolResult> => {
        ...
        if (
            ...
            throw new McpError(
                ...
                "Invalid or missing arguments for search_emergent_signals. Expected { query: string, numResults?: number, resultCategory?: string }."
            );
        }
        ...
        let category: ExaResultCategory | undefined;
        if (args.resultCategory !== undefined) {
            if (typeof args.resultCategory !== "string") {
                throw new McpError(
                    ErrorCode.InvalidParams,
                    "resultCategory must be a string if provided."
                );
            }

            const allowedCategories: Set<ExaResultCategory> = new Set([
                "company",
                "research_paper",
                "news_article",
                "pdf",
            ...
            if (!allowedCategories.has(args.resultCategory as ExaResultCategory)) {
                throw new McpError(
                    ErrorCode.InvalidParams,
                    `Invalid resultCategory "${args.resultCategory}".`
                );
            }

            category = args.resultCategory as ExaResultCategory;
        }
        ...
        try {
            ...
            console.error(
                `[AI Consumer Needs MCP] 🔍 search_emergent_signals query="${trimmedQuery}" numResults=${numResults} category=${category ?? "none"
                }`
            );
